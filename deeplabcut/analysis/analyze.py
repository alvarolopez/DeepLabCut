...
def main():
    ...
    experimentname = CONF.data.task + CONF.net.date + '-trainset' + str(
            int(CONF.analysis.trainings_fraction * 100)) + 'shuffle' + str(CONF.analysis.shuffle_index)
...
#    cfg = load_config(os.path.join(modelfolder , 'test' ,"pose_cfg.yaml"))
    ...
    cfg['init_weights'] = os.path.join(modelfolder , 'train', Snapshots[CONF.analysis.snapshot_index])
...
#    os.chdir(videofolder)
    videos = np.sort([fn for fn in os.listdir(videofolder) if (videotype in fn)])
    ...
    for video in videos:
        ...
        try:
            ...
            if CONF.data.cropping:
                clip = clip.crop(
                    y1=CONF.data.y1, y2=CONF.data.y2, x1=CONF.data.x1, x2=CONF.data.x2)  # one might want to adjust
