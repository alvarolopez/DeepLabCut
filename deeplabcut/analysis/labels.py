...
from deeplabcut.train.utils import auxiliaryfunctions
    ...
    for index in tqdm(range(nframes)):
        ...
        if os.path.isfile(tmpfolder + "/file%04d.png" % index):
            ...
            for bpindex, bp in enumerate(bodyparts2plot):
                if Dataframe[scorer][bp]['likelihood'].values[index] > CONF.evaluation.pcutoff:
                    plt.scatter(
...
#    os.chdir(tmpfolder)
    ...
    subprocess.call([
        'ffmpeg', '-framerate',
        str(clip.fps), '-i', os.path.join(tmpfolder, 'file%04d.png'), '-r', '30', os.path.join(videofolder, vname + '_DeepLabCutlabeled.mp4')])
...
def main():
    ...
    scorer = 'DeepCut' + "_resnet" + str(CONF.net.resnet) + "_" + CONF.data.task + str(CONF.net.date) + 'shuffle' + str(CONF.analysis.shuffle_index) + '_' + str(CONF.analysis.trainings_iterations)
...
#    os.chdir(videofolder)
    ...
    videos = np.sort([fn for fn in os.listdir(videofolder) if (videotype in fn) and ("labeled" not in fn)])
    print(videos)
