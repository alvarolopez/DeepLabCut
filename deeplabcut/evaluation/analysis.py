...
def pairwisedistances(DataCombined, DataMachine, scorer1,scorer2,pcutoff=-1,bodyparts=None):
...
def main():
    ...
    for trainFraction in CONF.net.training_fraction:
        for shuffle in CONF.net.shuffles:
            ...
            fns = [file for file in os.listdir(results_dir)
                if "forTask_" + str(CONF.data.task) in file and "shuffle" + str(shuffle) in
                file and "_" + str(int(trainFraction * 100)) in file]

            metadatafile = os.path.join(folder , "Documentation_" + CONF.data.task + "_" + str(
            ...
            for trainingiterations,index in snapindices:
              DataMachine = pd.read_hdf(os.path.join(results_dir, fns[index]), 'df_with_missing')
              DataCombined = pd.concat([Data.T, DataMachine.T], axis=0).T
              scorer_machine = DataMachine.columns.get_level_values(0)[0]
              MSE,MSEpcutoff = pairwisedistances(DataCombined, DataMachine, CONF.label.scorer, scorer_machine, CONF.evaluation.pcutoff,comparisonbodyparts)
              ...
              print("With pcutoff of", CONF.evaluation.pcutoff," train error:",np.round(trainerrorpcutoff,2), "pixels. Test error:", np.round(testerrorpcutoff,2), "pixels")

                #FIXME(aloga): review this!!!! need a display for plotting
#              if CONF.evaluation.plotting==True:
#                 foldername=os.path.join(results_dir, 'LabeledImages_'+scorer_machine)
